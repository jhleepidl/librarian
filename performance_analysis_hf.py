...
class VectorDBPerformanceAnalyzer:
    def __init__(self, full_db_dir: str, hf_model_name: str = "jhleepidl/librarian"):
        """
        Vector DB performance analyzer using Hugging Face Dynamic Direction-focused model
        
        Args:
            full_db_dir: Vector DB directory path
        ...
        try:
            ...
            try:
                ...
                # Try pytorch_model.bin if safetensors not available
                try:
                    model_path = hf_hub_download(
                        repo_id=hf_model_name,
            ...
            try:
                # Alternative: Load config.json first to check model structure
                from huggingface_hub import hf_hub_download
                ...
                raise RuntimeError(f"Failed to load Hugging Face model {hf_model_name}. Errors: {e}, {e2}")
    ...
    def get_api_embedding(self, tool_name: str, api_name: str) -> np.ndarray:
        """
        Get embedding for specific API
        Use ToolBench model to generate embedding from full text
    ...
    def calculate_threshold(self, test_data: List[Dict[str, Any]]) -> float:
        """
        Calculate threshold as minimum similarity between relevant APIs and queries
    ...
    def print_detailed_results(self, analysis_result: Dict[str, Any], 
        ...
        print("\n" + "=" * 80)
        print("VECTOR DB PERFORMANCE ANALYSIS RESULTS (Hugging Face Dynamic Direction-focused Model)")
        print("=" * 80)
...
def main():
    ...
    # Initialize performance analyzer (Hugging Face Dynamic Direction-focused model)
    ...
    # Save results
    output_file = "three_method_performance_comparison_hf.json"
